...
// <h1 class = "title" > { type: 'openTag' , name: 'h1' , attributions: { class: 'title'}} 
export type Token = {
    type: TokenType
    name?: string
...
// initial - not doing naything ( idle state ) , intag - currently reading and intext - currently reading text content between tags 
...
// this funciton takes the raw string stored in buffer and returns
// either a token or something null liek sorry humse nhi ho payega
function parseTag(raw: string): Token | null {
    raw = raw.trim() // rmeoves any accidental spaces from start and end , just a clean process to be safe 
